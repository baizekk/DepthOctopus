extends 状态节点
var 碰撞大小 = RectangleShape2D.new()
func 状态进入() -> void:
	print("进入小静止")
	动画.play("小静止")
	碰撞大小.size = Vector2(8, 8)
	碰撞.shape = 碰撞大小
func 状态退出() -> void:
	quanjv.体力计时 = false
	quanjv.力竭 = false
	
	
func 更新(delta : float) -> void:
	运动()
	
	quanjv.能量 -= 能量消耗 * delta
	if quanjv.能量 <= 0:
		quanjv.没蓝 = true
		quanjv.能量 = 0

		
	if quanjv.体力 <= 0:
		quanjv.力竭 = true
		quanjv.体力 = 0
	
	
func 物理更新(delta : float) -> void:
	主角.velocity = 主角.velocity.lerp(Vector2(0, -10), 0.03)
	
func 跳转条件() -> String:
		
	if not Input.is_action_pressed("跳跃") or quanjv.没蓝:
		print("静止")
		return "静止"
	
	elif not is_zero_approx(左右方向) and Input.is_action_pressed("跳跃") and quanjv.没蓝 == false:
		print("小移动")
		return "小移动"
		
	elif not is_zero_approx(上下方向) and Input.is_action_pressed("跳跃") and quanjv.没蓝 == false:
		print("小移动")
		return "小移动"
	
	elif quanjv.力竭 and Input.is_action_pressed("跳跃") and quanjv.没蓝 == false:
		print("小肥")
		return "小肥"
	
	else:
		return ""
